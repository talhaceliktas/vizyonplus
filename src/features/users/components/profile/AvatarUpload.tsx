...
// Supabase Client importunu projenin yapısına göre güncelledik
...
interface AvatarUploadProps {
  ...
  currentSrc: string | null; // Parent componentten bu isimle geliyor
}
...
export default function AvatarUpload({
  ...
  async function uploadAvatar(event: React.ChangeEvent<HTMLInputElement>) {
    try {
      ...
      // Cache sorunu olmaması için dosya ismine timestamp ekliyoruz
      const fileName = `avatar-${Date.now()}.jpg`;
      const filePath = `${user.id}/${fileName}`;

      const bucket = supabase.storage.from("profil_fotograflari");

      // Önceki resmi silmek istersen buraya silme kodu eklenebilir ama şart değil.
